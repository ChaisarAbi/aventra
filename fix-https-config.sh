...
# Script untuk memastikan semua konfigurasi menggunakan HTTPS
echo "🔧 Memperbaiki konfigurasi HTTPS untuk Aventra Portfolio..."

# 1. Update API configuration di frontend
echo "1. Memperbarui konfigurasi API di frontend..."
...
echo "2. Memperbarui konfigurasi Next.js..."
...
const nextConfig = {
  ...
  env: {
    NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL || 'https://api.aventra.my.id',
  },
}
...
# 3. Update Docker Compose untuk production
echo "3. Memperbarui Docker Compose production..."
...
services:
  ...
  backend:
    ...
    volumes:
      - ./backend/portfolio.db:/app/portfolio.db
      - ./backup:/app/backup
    networks:
      - aventra-network
    healthcheck:
